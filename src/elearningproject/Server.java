...
public class Server extends javax.swing.JFrame implements Runnable {
    public  static ArrayList<PaintPanel> pt=new ArrayList<>();
    public  static Stack<Shape> shapes=new Stack<>();
//    public  static HashMap<String,String> cours=new HashMap<>();
    public  static Stack<Shape> removed=new Stack<>();
...
//            DrawFrame frame=new DrawFrame();
    ...
    public void run() {
        while (true) {
            try {
                ...
                System.out.println("I am Server : Message received: " + message);
                    ...
                    if(message.contains("envoie a un client")){
                        ...
                        for(String key : k){
                            ...
                            if(key.equals(idClient)){
                                ...
                                System.out.println("je vais envoyer :"+msg);
                                byte[] buf = new byte[1024];
                                buf = msg.getBytes();
                                DatagramPacket packet1 = new DatagramPacket(buf, buf.length, clientAddress, Integer.parseInt(s[1]));
                                socket.send(packet1);
                                buf=null;
                            }
                        }
                        ...
                        DefaultListModel listdlm=null;
                        for (PaintPanel p : pt) {
                            System.out.println("p: "+p.name);
                            if(p.name.equals(sender)){
                                listdlm=p.getFrame().dlm;
                                break;
                            }
                        }
                        ...
                        while(itr.hasNext()){
                            ...
                            if(listdlm.contains(key)) {
                                try {
                                    ...
                                    new PrepareClientList("null", idClient).start();
                                } catch (Exception ex) {
    ...
    class PrepareClientList extends Thread{
        String profname;
        String name;
        public PrepareClientList(String profname,String name){
        ...
        public void run(){
            try{
                ...
                for (PaintPanel p : pt) {
                    System.out.println("p: "+p.name);
                    if(p.name.equals(this.profname)){
                        System.out.println("I found profname");
                        listdlm=p.getFrame().dlm;
                        listdlm.addElement(p.name);
                        listdlm.addElement(this.name);
                        break;
                    }
                }
                String ids="";
                Set k=clientColl.keySet();
//                Iterator itr=k.iterator();
                // iterate a default list model
                for (int i = 0; i < listdlm.size(); i++) {
                    ids=ids+listdlm.get(i)+",";
                }
...
//                }
    ...
    public static void main(String args[]) throws RemoteException, MalformedURLException {
        ...
        //</editor-fold>
        PainImpl pt=new PainImpl();
        LocateRegistry LocalRegistry = null;
        LocalRegistry.createRegistry(1099);
        Naming.rebind("paint",pt);
